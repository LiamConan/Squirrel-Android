...
class MainViewModel : ViewModel() {
	...
	fun addAccount(directoryIndex: Int, account: Account) {
		keysUseCases.addAccount(data.value?.get(directoryIndex)?.accounts ?: mutableListOf(), account)
	...
	fun swapDirectories(from: Int, to: Int): MutableList<Directory> {
		keysUseCases.swapDirectories(data.value ?: mutableListOf(), from, to)
		data.value = data.value
		return data.value ?: mutableListOf()
	}
}